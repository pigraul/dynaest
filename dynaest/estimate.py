...
from . import config, constants, estimation, dedup, utils
...
def complement_counts(df_counts: pd.DataFrame, gene_infos: dict) -> pd.DataFrame:
    ...
    columns = other_columns + dedup.COLUMNS
    df_forward = df_counts[forward_strand][columns]
    df_reverse = df_counts[~forward_strand][columns]

    df_reverse.columns = other_columns + dedup.CONVERSION_COLUMNS[::-1] + dedup.BASE_COLUMNS[::-1]
...
def estimate(
    count_dirs: List[str],
    out_dir: str,
    reads: Union[Literal['complete'], List[Literal['transcriptome']]] = 'complete',
    ...
    gene_infos, _ = ngs.gtf.genes_and_transcripts_from_gtf(genes_filename, use_version=False)
    ...
    for i, count_dir in enumerate(count_dirs):
        ...
        _df_counts = dedup.read_counts(counts_path)
        # Filter barcodes
        if barcodes:
    ...
    df_counts = complement_counts(df_counts_uncomplemented, gene_infos)
    logger.info(
    ...
    if control_p_e:
        ...
        if control:
            ...
            rates = utils.read_rates(os.path.join(count_dir, f'{constants.RATES_PREFIX}.csv'))
            if groups:
    ...
    for key in set(reads).union(['transcriptome'] if transcriptome_all else ['total']):
        ...
        df = dedup.subset_counts(df_counts, key)

        for convs in conversions:
            convs = sorted(convs)
            other_convs = list(set(all_conversions) - set(convs))
            aggregates_paths.setdefault(key, {})[tuple(convs)] = utils.aggregate_counts(
    ...
    for convs in conversions:
        ...
        df_aggregates = utils.read_aggregates(
    ...
    if method == 'pi_g':
        ...
        for key in reads:
            for convs in conversions:
                ...
                df_aggregates = utils.read_aggregates(aggregates_paths[key][tuple(convs)])
                if groups:
                    df_aggregates['group'] = df_aggregates['barcode'].map(groups).astype('category')
        ...
        for key in reads:
            for convs in conversions:
                ...
                alpha_path = estimation.estimate_alpha(
                    dedup.subset_counts(df_counts, key),
                    pi_c,
                    alpha_path,
                    conversions=convs,
                    group_by=[alpha_key],
